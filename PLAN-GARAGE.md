...
- `docs/shopify-nav-setup.md` link table: Category → `/<handle>` (e.g. `/lighting`; `/collections/<handle>` also works — rewritten). Replace the "CMS page" row with "Custom page → the route's path (pages are custom-built, Shopify CMS pages are not rendered)". Optional row: pre-filtered vehicle link `/<handle>/<make>/<model>/<year>` (rare — the redirect usually handles it).
