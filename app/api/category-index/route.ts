...
// Collection handle → canonical tree path, for `proxy.ts`.
//
// The proxy cannot call `use cache` functions, so it can't read the category
...
// here is a memory read. The proxy fetches this once per instance per minute
// and memoizes it.
//
...
export async function GET() {
  ...
  for (const [handle, node] of indexByHandle(await getCategoryTree())) {
    ...
    // would make the proxy redirect the page to itself, forever. This is the
